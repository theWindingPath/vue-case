...
<script type="text/ecmascript-6">
  ...
  export default { // 需要导出
    ...
    methods: { // methods是一个object，不是方法
      ...
      _drop(target) {
        // goods组件调用子组件shopcart里的方法drop()，vue里父组件调用子组件方法 v-ref:shopcart
        // this.$refs.shopcart.drop(target); // 调用子组件shopcart的drop方法，将target传进去

        // 利用 this.$nextTick() 接口，异步执行小球下落动画drop(),在向左平移动画执行完后做，优化性能
        this.$nextTick(() => { // 体验优化
