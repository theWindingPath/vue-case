      ...
      transitions: { // 动画属性
          drop: {
              ...
              enter(el) { // 每个钩子函数都接收一个参数el（需要执行动画的dom对象）,通过el可以访问到上面的<div transition="drop"></div>这个dom节点
                ...
                this.$nextTick(() => { // 用到$nextTcik这个接口，在下一次 event loop做什么事情，
                  // DOM 更新了，下面是DOM更新后要做事情,将样式重置回来
                      el.style.webkitTransiform = 'translate3d(0,0,0)'; // 样式重置
                      el.style.transform = 'translate3d(0,0,0)';
                      let inner = el.getElementsByClassName('inner-hook')[0];
                      inner.style.webkitTransiform = 'translate3d(0,0,0)';
              ...
              afterEnter(el) {
                ...
                //   console.log(this.balls);
                //   console.log(this.dropBalls);
                //   console.log(this.balls[0] === this.dropBalls[0]); // 打印为 true
                //   console.log(this.balls[0] === this.dropBalls[1]); // 打印 false

                  // this.balls 和 this.dropBalls 都可以获得那个小球dom节点，各自都可以操作ball节点
                  // this.dropBalls[0] = '改变了'; // this.dropBalls[0]改变了，但this.balls[0]没有随着改变，说明this.dropBalls和this.balls指向的作用域不同
                //   console.log(this.balls);
                //   console.log(this.dropBalls);
                //   console.log(this.balls[0] === this.dropBalls[0]); // 打印为 false
              }
          }
      }
  };
  ...
  .shopcart
      ...
      .ball-container
        .ball
          ...
          z-index 200
          &.drop-transition
            transition all 0.5s cubic-bezier(0.28, -0.51, 0.78, 0.29) // bezier曲线
            .inner
              width 16px
              height 16px
              border-radius 50%
